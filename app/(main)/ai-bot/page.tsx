...
const DUMMY_RESULT = {
  ...
  feedback: {
    speech: {
      score: 68,
      feedback:
        "질문과 설명의 비율이 30:70으로 설명에 치우쳐 있습니다. 고객에게 더 많은 질문을 던져보세요.",
      suggestion: "SPIN 기법을 활용해 상황-문제-시사-필요 순서로 질문해보세요.",
    },
    tone: {
      score: 75,
      feedback:
        "전반적으로 안정적인 톤이지만, 2분 30초 부근에서 말 속도가 급격히 빨라집니다.",
      suggestion:
        "긴장되는 순간에도 일정한 페이스를 유지하는 연습이 필요합니다.",
    },
    closing: {
      score: 60,
      feedback:
        "고객의 구매 신호가 3회 감지되었으나 클로징 시도가 0회입니다. 기회를 놓치고 있습니다.",
      suggestion:
        '"다음 단계로 넘어갈까요?" 같은 자연스러운 클로징 문구를 준비해두세요.',
    },
    objection: {
      score: 78,
      feedback:
        "가격 반론에 대해 기능 설명으로 대응했습니다. 나쁘지 않지만 더 효과적인 방법이 있습니다.",
      ...
      <div className="space-y-4 px-4 py-4">
        ...
        <Card className="bg-gradient-to-br from-[#1B3A5C] to-[#2E75B6] text-white">
          <CardContent className="p-6 text-center space-y-2">
            ...
            <p className="text-sm text-white/80">
              {DUMMY_RESULT.feedback.overall.feedback}
            </p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          {SCORE_ITEMS.map(({ key, label, icon: Icon }) => {
            const score =
              DUMMY_RESULT[
                `score_${key}` as keyof typeof DUMMY_RESULT
              ] as number;
            return (
              <Card key={key}>
        ...
        {SCORE_ITEMS.map(({ key, label, icon: Icon }) => {
          const fb =
            DUMMY_RESULT.feedback[key as keyof typeof DUMMY_RESULT.feedback];
          return (
            <Card key={key}>
        ...
        <>
          ...
          <Card className="bg-amber-50 border-amber-200">
            <CardContent className="p-3 text-xs text-amber-800 space-y-1">
              ...
              <p>
                녹음에 고객명, 전화번호 등 개인정보가 포함되어 있다면 직접 편집
                후 업로드해주세요.
              </p>
          ...
          <div className="text-center text-sm text-muted-foreground">
            오늘 남은 무료 분석: <span className="font-bold">1회</span>
          </div>
